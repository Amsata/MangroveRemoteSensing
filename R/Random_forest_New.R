...
for (i in 1:nlayers(data_ldst_8)) {
  ...
  filename<-paste0("E:\\remote sensing with R\\RemoteSensing\\data\\bands\\Band",i,".tif")
...
#data_ldst_8<-raster("indices/data_ldst_8.tif")
...
covs<- raster::stack(NDVI,NDWI,BSI,AVI,NDSMI,AMSA,R62,MSAVI,NRI,SI1,SI5,SMI,SRPI,NDII)
names(covs) <- c('ndvi', 'ndwi','bsi','avi','ndsmi','amsa','r62', 'msavi',
                 'nri','si1','si5','smi','srpi','ndii')
...
write.csv(valuetable,"donnee_04152340.csv")

# g1<-ggplot(valuetable,aes(y=ndvi,fill=as.factor(class)))+
#   geom_boxplot()
# 
