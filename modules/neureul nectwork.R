...
for (i in 1:length(covs)) {
  ...
  names(covs[[i]]) <- c('ndvi', 'ndwi','bsi','avi','amsa')
}
...
subset_vect<-c(3,8,9,10,12,13,17)
...
training <- point.df[ind==1, 1:vv]
test <- point.df[ind==2, 1:vv]
# Split the class attribute
trainingtarget <- as.numeric(point.df[ind==1, (vv+1)])
testtarget <- point.df[ind==2, (vv+1)]
